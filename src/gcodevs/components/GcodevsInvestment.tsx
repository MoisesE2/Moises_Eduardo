...
const GcodevsInvestment: React.FC = () => (
  <section id="investimento" className="bg-slate-50 py-20 px-4 sm:px-6">
        ...
        <table className="w-full text-sm min-w-[640px]">
              ...
              {SERVICES.map((service) => (
                <th
                  ...
                  className={`p-4 text-center font-extrabold text-base ${
                    service.highlighted ? "text-violet-700" : "text-slate-900"
                  }`}
                >
                  {service.name}
                  {service.highlighted && (
                    <span className="block text-[11px] font-bold uppercase tracking-wide text-violet-500">
                      mais pedido
                    </span>
                  )}
                </th>
          ...
          <tbody>
            {INVESTMENT_ROWS.map((row, rowIndex) => (
              <tr key={row.label} className={rowIndex % 2 === 0 ? "bg-slate-50/60" : "bg-white"}>
                <td className="p-4 font-semibold text-slate-700">{row.label}</td>
                {row.values.map((value, colIndex) => (
                  <td
                    key={colIndex}
                    className={`p-4 text-center text-slate-600 ${
                      SERVICES[colIndex].highlighted ? "bg-violet-50/40" : ""
                    }`}
                  >
                    {value}
                  </td>
